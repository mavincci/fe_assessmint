  ...
  const handleDescriptionChange = (e) => {
    ...
    const questionsData = useSelector((state)=> state.bankreducer.BankRepository?.body)
        useEffect(() => {
          ...
          await dispatch(load_my_question_Bank());
          };
  ...
  const filteredQuestions = useMemo(() => {
    ...
    console.log("State",state)
      ...
      const matchesCategory =
        ...
        question.category?.name === state.selectedCategory;
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            ...
            <div className="md:ml-auto w-full md:w-auto flex flex-wrap gap-3 mt-3 md:mt-0">
              ...
              {/* Category filter */}
   <select
  className="select select-bordered w-full md:w-[160px]"
  value={state.selectedCategoryId}
  onChange={(e) => {
    const selected = JSON.parse(e.target.value);
    updateField("selectedCategory", selected.name);     
    updateField("selectedCategoryId", selected.id);       
  }}
>
  <option value={JSON.stringify({ id: "All categories", name: "All Categories" })}>
    All Categories
  </option>

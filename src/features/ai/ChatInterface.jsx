  ...
  const [messages, setMessages] = useState([
    {
      id: '1',
      type: 'system',
      content: 'Hello! I can generate quiz questions for you. Try asking for true/false or multiple choice questions about any topic.',
...
const handleSendMessage = async (message) => {
  ...
  const handleEditTFQuestion = (id, updatedQuestion) => {
    setTfQuestions((prev) => prev.map((q, index) => (index === id ? updatedQuestion : q)));
  };
  ...
  const handleEditMCQQuestion = (id, updatedQuestion) => {
    setMcqQuestions((prev) => prev.map((q, index) => (index === id ? updatedQuestion : q)));
  };
      ...
      <header className="bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700 transition-colors duration-300 bg-grad">
        <div className="container mx-auto px-4 py-3 flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-semibold text-gray-800 dark:text-white">AssessMint AI Question Generator</h1>
          </div>
          {/* <button
            onClick={toggleTheme}
      ...
      <div className="bg-white dark:bg-gray-800 shadow-sm transition-colors duration-300">
        <div className="container mx-auto px-4">
          <div className="tabs tabs-bordered p-1">
            ...
            >
              T/F Questions {tfQuestions?.length > 0 && `(${tfQuestions?.length})`}
            </button>
            <button
              className={`tab  ${activeTab === 'mcq' ? 'bg-accent-teal-light font-bold text-white rounded-full ' : ''}`}
              onClick={() => setActiveTab('mcq')}
            >
              MCQ Questions {mcqQuestions?.length > 0 && `(${mcqQuestions?.length})`}
            </button>
          </div>
        </div>
      </div>
          ...
          <div className="h-full flex flex-col pb-4">
            {activeTab === 'chat' && (
                  ...
                  <div
                    key={message.id}
                    className={`flex ${
                      message.type === 'user' ? 'justify-end' :message.type === 'assistant'? 'justify-start':"justify-center my-10 text-white"
                    } animate-fade-in`}
                  >
                    <div
                        ...
                        <div>
                          ...
                          {message.hasQuestions && (
                            <div className="mt-2 flex items-center text-sm text-gray-500 dark:text-gray-400">
                              ...
                              <span>View in the {message.questionType === 'tf' ? 'T/F Questions' : 'MCQ Questions'} tab</span>
                            </div>
                          )}
                        </div>
                      </div>
                ...
                {loading && (
                  <div className="flex justify-start animate-fade-in">
                    <div className="max-w-3xl rounded-lg px-4 py-3 bg-white dark:bg-gray-800 shadow border border-gray-200 dark:border-gray-700">
                      ...
                      <img src={aiImage}  className='h-5 w-5 animate-spin duration-1000'/>

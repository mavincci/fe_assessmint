...
import {
  create_send_invitation,
  load_invited_candidates_by_assessment_ID,
} from "../../action/Auth";
...
const InvitePeople = ({ create_send_invitation }) => {
  ...
  const fetch_candidates_by_ID = async () => {
    ...
    const res = await dispatch(
      load_invited_candidates_by_assessment_ID(assessmentId)
    );
    if (res?.body) {
      ...
      <div className=" md:w-1/2 space-y-4">
        ...
        <div className="card bg-base-100 border rounded border-gray-200 p-4">
          <div className="form-control flex flex-col">
            ...
            <input
              type="text"
              className="font-bold text-lg text-accent-teal-light"
              disabled
              value={Name}
            />
          </div>
        ...
        <div className="flex-row border rounded border-gray-200 p-4">
          <form
            ...
            className="flex items-center flex-col last:items-end gap-4"
          >
            <textarea
            ...
            <div className="card bg-base-100 border rounded border-gray-200 p-4 w-full flex-row gap-3 flex-wrap max-h-44 overflow-y-scroll scrollbar-hide">
              {invites.length > 0 ? (
                invites.map((email, index) => (
                  <div
                    key={index}
                    className="flex flex-row items-center py-2 border border-gray rounded-xl p-2 gap-3  w-fit "
                  >
                    <p className="text-sm">{email}</p>
                    <div className=" text-sm flex flex-row gap-2 rounded-l-full md:p-1">
                      <Edit
                        className="cursor-pointer text-accent-teal-light h-5 w-5"
                        onClick={() => handleEdit(index)}
                      />
                      <X
                        className="cursor-pointer text-orange-500 h-5 w-5"
        ...
        <div className="flex justify-end gap-4">
          ...
          <button
            className="btn bg-btn-primary text-white"
...
export default connect(mapStateToProps, { create_send_invitation })(
  InvitePeople
);

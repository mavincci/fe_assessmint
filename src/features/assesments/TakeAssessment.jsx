...
const TakeAssessment = () => {
  ...
  const [startAssesment, setStartAssesment] = useState(false);
    ...
    const totalQuestions = testquestions.reduce((sum, quiz) => sum + quiz.questions.length, 0)
    ...
    const handlePrevious = () => {
      if (currentQuestionIndex > 0) {
        ...
        setCurrentQuestionIndex(testquestions[currentQuizIndex - 1].questions.length - 1)
      }
    }
  ...
  <div className="bg-bg-secondary-light rounded-lg shadow-lg p-8 max-w-xl w-full ">
    ...
    />
            ...
            {startAssesment &&  <div className="min-h-screen bg-sky-50 p-4">
        {/* Header */}
          ...
          <div className="bg-white rounded-lg shadow-md p-6 flex-1">
            ...
            <div className="w-full bg-gray-200 rounded-full h-2 mb-6">
              <div
                ...
                style={{ width: `${(currentOverallQuestionNumber / totalQuestions) * 100}%` }}
              ></div>
            </div>
            ...
            <div className="mb-6">
              ...
              <div className="space-y-3">
                {currentQuestion.choices.map((choice, index) => (
                  ...
                  >
                    <span className="font-medium">{getOptionLetter(index)})</span> {choice}
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="flex justify-between mt-8">
              ...
              <button
                ...
                onClick={handleNext}
                disabled={currentQuizIndex === totalQuizzes - 1 && currentQuestionIndex === totalQuestionsInQuiz - 1}
              >
                Next
              </button>
            </div>
              ...
              {allQuestions.map((question, index) => (
                <div
                  ...
                  className={`border border-dashed border-blue-300 rounded flex items-center justify-center h-12 cursor-pointer hover:bg-blue-50 ${
                    currentQuizIndex === question.quizIndex && currentQuestionIndex === question.questionIndex
                      ? "bg-blue-100"
                      : userAnswers[question.id]
                        ? "bg-blue-50"
                        : "bg-white"
                  }`}
                  onClick={() => handleQuestionClick(question.quizIndex, question.questionIndex)}
                >
                      <span>{index + 1}</span>
                        ...
                        <div className="flex flex-col gap-4 ">
                            <Button icon={<BookmarkCheck />} label="Finish Assesment" bg="bg-btn-primary" text="text-white" onClick={() => setStartAssesment(!startAssesment)} />
                            <Button icon={<LogOut />} label="Exit Assesment" bg="bg-error" onClick={() => setStartAssesment(!startAssesment)} />
                       </div>
